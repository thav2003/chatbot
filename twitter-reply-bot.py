...
class TwitterBot:
    def __init__(self):
        ...
        self.tweet_response_limit = (
            35  # How many tweets to respond to each time the program wakes up
        )
    ...
    def get_mention_conversation_tweet(self, mention):
        ...
        if mention.conversation_id is not None:
            conversation_tweet = self.twitter_api.get_tweet(
    ...
    def check_already_responded(self, mentioned_conversation_tweet_id):
        ...
        for record in records:
            if record["fields"].get("mentioned_conversation_tweet_id") == str(
    ...
    def execute_replies(self):
        ...
        print(
            f"Finished Job: {datetime.utcnow().isoformat()}, Found: {self.mentions_found}, Replied: {self.mentions_replied}, Errors: {self.mentions_replied_errors}"
        )
